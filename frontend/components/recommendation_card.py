    ...
    *,
    ...
    with st.container():
        ...
        with col_badge:
            ...
            mc_bg, mc_border, mc_text = _mc.get(
                market_type, ("#f5f5f5", "#adb5bd", "#6c757d")
            )
        ...
        c4.metric(
            "Latent Demand",
            _signal_label(latent_demand_score)
            if latent_demand_score is not None
            else "—",
        ...
        if cluster_confidence is not None:
            try:
                ...
                if cc < 0.25:
                    st.warning(
                        f"⚠ Borderline cluster (confidence {cc:.2f}) — market type may shift with new data."
                    )
            except (TypeError, ValueError):
                pass
        try:
            ...
            st.plotly_chart(
                _gauge, use_container_width=True, config={"displayModeBar": False}
            )
        except Exception:
        ...
        with st.expander("Score breakdown", expanded=False):
            try:
                contrib_fig = go.Figure(
                    go.Bar(
                        x=[
                            0.4 * float(demand_score),
                            0.4 * float(gap_score),
                            0.2 * float(viability_score),
                        ],
                        y=["Demand signal", "Supply gap", "Viability"],
                        orientation="h",
                        marker_color=["#e63946", "#2a9d8f", "#457b9d"],
                        text=[
                            f"{0.4 * float(demand_score):.3f}",
                            f"{0.4 * float(gap_score):.3f}",
                            f"{0.2 * float(viability_score):.3f}",
                        ],
        ...
        with st.expander("Next-Year Outlook", expanded=False):
            if halal_demand_forecast is not None:
                ...
                if val > 0.5:
                    st.success(f"Trending Up: {val:.1%}")
                elif val > 0.3:
                    st.info(f"Steady: {val:.1%}")
                else:
                    st.warning(f"Weak: {val:.1%}")
            else:
