...
def process_demographics():
    ...
    try:
        ...
        nta_col = (
            ...
            else next((c for c in df.columns if "NTA" in str(c).upper()), None)
        )
        pop_col = (
            "Pop1"
            if "Pop1" in df.columns
            else next((c for c in df.columns if "POP20" in str(c).upper()), None)
        )
        ...
        if nta_col and pop_col:
            ...
            extra_mappings = [
                {
                    "nta_id": "MN22",
                    "population": df[df["GeoID"] == "MN0202"]["Pop1"].values[0]
                    if "MN0202" in df["GeoID"].values
                    else 30000,
                },
                {
                    "nta_id": "MN23",
                    "population": df[df["GeoID"] == "MN0203"]["Pop1"].values[0]
                    if "MN0203" in df["GeoID"].values
                    else 25000,
                },
                {
                    "nta_id": "MN25",
                    "population": df[df["GeoID"] == "MN0101"]["Pop1"].values[0]
                    if "MN0101" in df["GeoID"].values
                    else 60000,
                },
                {
                    "nta_id": "BK31",
                    "population": df[df["GeoID"] == "BK0602"]["Pop1"].values[0]
                    if "BK0602" in df["GeoID"].values
                    else 40000,
                },
            ]
            ...
            final_result = pd.concat(
                [result[["nta_id", "population"]], mapping_df], ignore_index=True
            )

            final_result["nta_id"] = final_result["nta_id"].astype(str)
            final_result["population"] = pd.to_numeric(
                final_result["population"], errors="coerce"
            )
            final_result = final_result.dropna(subset=["population"])

            final_result.to_csv(output_path, index=False)
            logger.info(
                f"Successfully saved {len(final_result)} NTA demographic records (including manual mappings) to {output_path}"
            )
        else:
            logger.error("Could not find NTA or Population columns.")
